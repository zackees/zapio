...
class ProcessTracker:
    ...
    def register_process(
        ...
        self._save_registry()
        logging.info(
            f"Registered process tree: client={client_pid}, root={root_pid}, children={len(self._registry[client_pid].child_pids)}, operation={operation_type}"
        )
    ...
    def unregister_process(self, client_pid: int) -> None:
        ...
        with self.lock:
            if client_pid in self._registry:
                ...
                logging.info(
                    f"Unregistered process tree: client={client_pid}, root={info.root_pid}"
                )
    ...
    def _update_child_pids(self, client_pid: int) -> None:
        ...
        try:
            ...
            logging.debug(
                f"Updated child PIDs for client={client_pid}: {len(info.child_pids)} children"
            )
    ...
    def cleanup_orphaned_processes(self) -> list[int]:
        ...
        with self.lock:
            for client_pid, info in list(self._registry.items()):
                ...
                logging.info(
                    f"Client {client_pid} is dead, cleaning up process tree (root={info.root_pid}, children={len(info.child_pids)}, operation={info.operation_type})"
                )
