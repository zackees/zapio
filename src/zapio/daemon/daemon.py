...
def process_deploy_request(
    ...
    logging.info(
        f"Processing deploy request {request_id}: env={environment}, project={project_dir}, port={port}"
    )
...
def process_monitor_request(
    ...
    logging.info(
        f"Processing monitor request {request_id}: env={environment}, project={project_dir}, port={port}"
    )
...
def signal_handler(signum: int, frame: object) -> None:
    ...
    with _operation_lock:
        if _operation_in_progress:
            logging.warning(
                f"Received {signal_name} during active operation. Refusing graceful shutdown."
            )
            print(
